...
export async function validatePlugin(
	...
	try {
		...
		if (
			...
			result.errors.push(
				"Missing required .claude-plugin/plugin.json file"
			);
		} else if (error instanceof SyntaxError) {
			result.errors.push(
				`Invalid JSON in .claude-plugin/plugin.json: ${error.message}`
			);
		} else {
			result.errors.push(
	...
	if (!pluginJson.name) {
		...
		if (!/^[a-z][a-z0-9]*(-[a-z0-9]+)*$/.test(pluginJson.name)) {
			result.warnings.push(
				`Plugin name '${pluginJson.name}' should be kebab-case`
			);
		}
	}
	...
	if (pluginJson.version) {
		if (typeof pluginJson.version !== "string") {
			...
			if (!/^\d+\.\d+\.\d+/.test(pluginJson.version)) {
				result.warnings.push(
					`Plugin version '${pluginJson.version}' should be semver format`
				);
			}
		}
	}
	...
	for (const { key, value } of componentPaths) {
		if (value) {
			...
			if (!value.startsWith("./")) {
				result.warnings.push(
					`plugin.json '${key}' path '${value}' should start with './'`
				);
			}
			...
			try {
				...
				result.warnings.push(
					`plugin.json '${key}' path '${value}' does not exist`
				);
			}
		}
	}
		...
		if (hooksStats.isDirectory()) {
			...
			try {
				...
				if (
					...
					result.warnings.push(
						"hooks/ directory exists but hooks/hooks.json is missing"
					);
				} else if (error instanceof SyntaxError) {
					result.errors.push(
						`Invalid JSON in hooks/hooks.json: ${error.message}`
					);
					result.valid = false;
				}
			}
...
async function validateHooksJson(
	...
	for (const [hookName, hookConfig] of Object.entries(hooks)) {
		if (!hookConfig || typeof hookConfig !== "object" || Array.isArray(hookConfig)) {
			result.warnings.push(
				`Hook '${hookName}' configuration should be an object`
			);
			continue;
		}
		...
		if (typeof configCommand === "string") {
			if (
				configCommand.includes("/") &&
				!configCommand.startsWith("${CLAUDE_PLUGIN_ROOT}")
			) {
				result.warnings.push(
			...
			for (const hookDef of configHooks) {
				if (hookDef && typeof hookDef === "object") {
					...
					if (typeof defCommand === "string") {
						if (
							defCommand.includes("/") &&
							!defCommand.startsWith("${CLAUDE_PLUGIN_ROOT}")
						) {
							result.warnings.push(
 ...
 */
export function checkPluginNameCollisions(
 ...
 */
export async function validatePluginsWithCollisionCheck(
	pluginDirs: string[]
): Promise<{
	results: PluginValidationResult[];
	collisions: string[];
